...
async function rehydrateCover(post) {
  const u = post.cover && post.cover.url;
  if (!u) return;
  mkdirSync('assets/posts', { recursive: true });

  // (a) base64 data URI (e.g. a pasted image) -> decode to a real file.
  ...
  // (b) remote http(s) cover (e.g. BabyLoveGrowth hero) -> fetch once and cache
  //     onto our own domain so og:image lives on blog.qualitum.ai and survives
  //     upstream URL changes. On any failure, keep the original URL (build never breaks).
  if (/^https?:\/\//.test(u)) {
    try {
      ...
      post.cover.url = '/' + rel;
      post.seo.ogImage = '/' + rel;
      console.log(`  • cached cover for ${post.slug} (${(buf.length/1024).toFixed(0)} KB)`);
    } catch (e) {
      console.log(`  • cover fetch failed for ${post.slug}, keeping remote URL (${e.message})`);
    }
  }
}
...
function renderArticle(post, ch) {
  ...
  const wc = post.bodyHtml.replace(/<[^>]+>/g,' ').split(/\s+/).filter(Boolean).length;
  const ogTags = (post.tags||[]).map(t=>`<meta property="article:tag" content="${esc(t)}" />`).join('');
  const ld = [
    { '@context':'https://schema.org','@type':'BlogPosting', headline:post.title, description:desc, image:[ogImg],
      ...
      author:{'@type':'Organization',name:post.author||'Qualitum',url:M+'/'},
      publisher:{'@type':'Organization',name:'Qualitum',url:M+'/',logo:{'@type':'ImageObject',url:M+'/assets/qualitum-logo.png'}},
      mainEntityOfPage:{'@type':'WebPage','@id':url}, isPartOf:{'@type':'Blog','@id':O+'/#blog'},
      inLanguage:'en', keywords:(post.tags||[]).join(', '), articleSection:(post.tags&&post.tags[0])||'Validation', wordCount:wc },
...
<meta property="article:modified_time" content="${new Date(post.updatedDate||post.publishedDate).toISOString()}" />
<meta property="article:publisher" content="${M}/" />${ogTags}
<meta name="keywords" content="${esc((post.tags||[]).join(', '))}" />
<meta name="author" content="${esc(post.author||'Qualitum')}" />
...
function renderIndex(posts, ch) {
  ...
  const graph = { '@context':'https://schema.org','@graph':[
    { '@type':'Organization','@id':M+'/#org', name:'Qualitum', url:M+'/', logo:{'@type':'ImageObject',url:M+'/assets/qualitum-logo.png'},
      description:'The private, LLM-agnostic agentic AI platform for life-sciences validation, quality and compliance.',
      sameAs:['https://www.linkedin.com/company/qualitum','https://x.com/cyberpartner_'] },
    { '@type':'WebSite','@id':O+'/#website', url:O+'/', name:'Qualitum Blog', publisher:{'@id':M+'/#org'}, inLanguage:'en' },
    { '@type':'Blog','@id':O+'/#blog', url:O+'/', name:'Qualitum Blog',
      description:'Field notes on private agentic AI for life-sciences validation, quality, and compliance.',
      publisher:{'@id':M+'/#org'}, inLanguage:'en',
      blogPost: posts.map(p=>({'@type':'BlogPosting',headline:p.title,url:`${O}/${p.slug}.html`,datePublished:new Date(p.publishedDate).toISOString(),description:descOf(p)})) },
    { '@type':'ItemList', itemListElement: posts.map((p,i)=>({'@type':'ListItem',position:i+1,url:`${O}/${p.slug}.html`,name:p.title})) }
  ]};
  const html = `<!DOCTYPE html><html lang="en"><head>
...
# Full-text feed for AI engines: ${O}/llms.txt and ${O}/llms-full.txt
`);
...
`);
  ...
  // llms-full.txt — complete article text for generative engines (GEO)
  const toText = (html) => html
    .replace(/<\/(p|h2|h3|li|blockquote|tr)>/g,'\n')
    .replace(/<[^>]+>/g,'').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>')
    .replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/\n{3,}/g,'\n\n').trim();
  const full = `# Qualitum Blog — full text\n\n> Field notes on private agentic AI for life-sciences validation, quality, and compliance. Published by Qualitum (${M}). Articles below are provided in full to help AI engines cite them accurately.\n\n` +
...
(async () => {
  ...
  await Promise.all(posts.map(rehydrateCover));
  const ch = chrome();
  let faq = 0;
  posts.forEach(p => { faq += renderArticle(p, ch); console.log(`  → /${p.slug}.html`); });
  renderIndex(posts, ch);
  meta(posts);
  console.log(`\n✓ Blog built: ${posts.length} post(s), ${faq} FAQ block(s) — index, rss, sitemap, robots, llms.txt, llms-full.txt.`);
})();
